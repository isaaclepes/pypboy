...
class Radio(game.Entity):
    ...
    def render(self, *args, **kwargs):
        ...
        text = selectFont.render(game.Entity.name, True, (config.bright), (0, 0, 0))
        
        #text = selectFont.render(" -   Random Play Radio ", True, (105, 251, 187), (0, 0, 0))
        
        self.blit(text, (75, 75))
        text = basicFont.render("  'r' selects a random song ", True, (config.bright), (0, 0, 0))
        self.blit(text, (75, 100))
        text = basicFont.render("  'p' to play   's' to stop ", True, (config.bright), (0, 0, 0))
        self.blit(text, (75, 120))
        
        if self.filename:
            text = selectFont.render(" %s " % metadata["ARTIST"] + ' - ' + metadata["TITLE"], True, (config.bright), (0, 0, 0))
            
            #text = selectFont.render(u" %s " % self.filename[self.filename.rfind(os.sep)+1:], True, (config.bright), (0, 0, 0))
